  ...
  <div class="flex min-h-screen bg-[#0a1628]">
    ...
    >
      ...
      <nav class="flex flex-1 flex-col gap-1 overflow-y-auto px-3 py-4">
        ...
        <NuxtLink to="/surat-keluar" class="nav-item active"><LucideSend :size="17" /><span>Surat Keluar</span></NuxtLink>
    ...
    <div class="flex flex-1 flex-col lg:pl-64">
      <header class="sticky top-0 z-30 flex items-center gap-3 border-b border-white/[0.07] bg-[#0a1628]/80 px-5 py-3.5 backdrop-blur-xl">
        ...
        <div class="flex-1">
          <h1 class="text-base font-bold text-slate-100">Arsip Surat Keluar</h1>
          <p class="text-xs text-slate-500">Nomor surat digenerate otomatis oleh sistem</p>
        </div>
      </header>
      ...
      <main class="flex flex-col gap-5 p-5">
        ...
        <div class="glass-card p-6">
          ...
          <form class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4" @submit.prevent="tambahSuratKeluar">
            <div class="flex flex-col gap-1.5">
              <label class="text-xs font-semibold text-slate-400">Tujuan Surat</label>
              <input v-model="form.tujuanSurat" type="text" class="form-input-base !pl-4" placeholder="Nama tujuan" :disabled="sedangSimpan" />
            </div>
            <div class="flex flex-col gap-1.5">
              <label class="text-xs font-semibold text-slate-400">Tanggal Kirim</label>
              <input v-model="form.tanggalKirim" type="date" class="form-input-base !pl-4" :disabled="sedangSimpan" />
            </div>
            <div class="flex flex-col gap-1.5">
        ...
        <div class="glass-card overflow-hidden">
          <div class="px-5 py-4 border-b border-white/[0.07] flex items-center justify-between">
            <h3 class="text-sm font-bold text-slate-200">Daftar Surat Keluar</h3>
            ...
            <table class="w-full text-sm">
              <thead>
                <tr class="border-b border-white/[0.07] text-left">
                  ...
                  <th class="px-5 py-3 text-[10px] font-bold uppercase tracking-wider text-slate-500">Tujuan Surat</th>
                  <th class="px-5 py-3 text-[10px] font-bold uppercase tracking-wider text-slate-500">Tanggal Kirim</th>
              ...
              <tbody>
                ...
                <tr v-else-if="daftarSurat.length === 0" class="border-b border-white/[0.05]">
                  <td colspan="5" class="px-5 py-8 text-center text-slate-500">Belum ada surat keluar.</td>
                </tr>
                <tr v-else v-for="(surat, i) in daftarSurat" :key="surat.id" class="border-b border-white/[0.05] hover:bg-white/[0.03] transition">
                  ...
                  <td class="px-5 py-3 font-mono font-medium text-seulanga-400">{{ surat.noSurat }}</td>
                  <td class="px-5 py-3 text-slate-300">{{ surat.tujuanSurat }}</td>
                  <td class="px-5 py-3 text-slate-400">{{ formatTanggal(surat.tanggalKirim) }}</td>
...
async function fetchData() {
  ...
  try {
    const res = await $fetch<any>('/api/surat/keluar', { headers: { Authorization: `Bearer ${authStore.token}` } })
...
async function tambahSuratKeluar() {
  ...
  try {
    const res = await $fetch<any>('/api/surat/keluar', {
      method: 'POST',
    ...
    form.tujuanSurat = ''; form.tanggalKirim = ''; form.keterangan = ''
    await fetchData()
    setTimeout(() => { pesanSukses.value = '' }, 5000)
  }
  catch (err: any) { pesanError.value = err.data?.statusMessage || 'Gagal menyimpan data.' }
