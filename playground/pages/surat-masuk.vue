...
const form = reactive({ tanggal: '', noSurat: '', pengirim: '', penerima: '', isiSurat: '', ditandaTanganiOleh: '' })
const sedangSimpan = ref(false)
const sedangMuat = ref(true)
const pesanSukses = ref('')
const pesanError = ref('')
interface SuratMasukItem { id: number, tanggal: string, noSurat: string, pengirim: string, penerima: string, isiSurat: string, jenisSurat: string, userPembuat: string, ditandaTanganiOleh: string | null, approvalTime: string | null, ket: string | null }
...
async function tambahSuratMasuk() {
  ...
  if (!form.tanggal || !form.noSurat || !form.pengirim || !form.penerima || !form.isiSurat) {
    pesanError.value = 'Tanggal, Nomor Surat, Pengirim, Penerima, dan Isi Surat wajib diisi.'
    return
  }
  ...
  try {
    await $fetch('/api/surat/masuk', {
      method: 'POST',
      headers: { Authorization: `Bearer ${authStore.token}` },
      body: { tanggal: form.tanggal, noSurat: form.noSurat, pengirim: form.pengirim, penerima: form.penerima, isiSurat: form.isiSurat, ditandaTanganiOleh: form.ditandaTanganiOleh || null },
    })
